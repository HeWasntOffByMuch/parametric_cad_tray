...
An export that wrote a 3MF comes back with `print_ledger`: the same geometry
priced under every infill option, and the regions that put density back where it
is load-bearing. It renders under the download list, because it is the answer to
a question the download itself raises — *is this the one I want to print?* — and
because the comparison only means anything next to the file it describes.

The option that writes no settings at all has no number, and the table shows a
dash rather than an estimate. What that costs is whatever preset the reader has
loaded, and the backend will not invent a figure about someone else's printer.
